...
__main_loop:
    ...
    if (pthread_create( &gUbusThreadID, 0, ubus_main, NULL) != 0) {
        dbg_time("%s Failed to create UbusThread: %d (%s)", __func__, errno, strerror(errno));
            return 0;
    }
...
__main_quit:
	...
	pthread_kill(gUbusThreadID, SIGQUIT);

    if (pthread_join(gUbusThreadID, NULL)) {
        dbg_time("%s Error joining to listener ubus thread (%s)", __func__, strerror(errno));
    }
