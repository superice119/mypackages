...
#if 0
int requestGetMSISDN(void) { //RIL_REQUEST_GET_IMSI
    PQCQMIMSG pRequest;
    PQCQMIMSG pResponse;
    PQMUX_MSG pMUXMsg;
	
	//static char MSISDN[16] = {'\0'};
	
	//PQCTLV_DEVICE_VOICE_NUMBERS pMSISDN;
	PQMIDMS_GET_MSISDN_RESP_MSG p;
	PQCTLV_DEVICE_VOICE_NUMBERS p1;

	int err;

	pRequest = ComposeQMUXMsg(QMUX_TYPE_DMS, QMIDMS_GET_MSISDN_REQ, NULL, NULL);
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check_and_return();

    p = (PQMIDMS_GET_MSISDN_RESP_MSG)(((PCHAR)&pMUXMsg->GetMsisdnRsp));
    p1 = (PQCTLV_DEVICE_VOICE_NUMBERS)(((PCHAR)&pMUXMsg->GetMsisdnRsp) + sizeof(PQMIDMS_GET_MSISDN_RESP_MSG));

    //pMSISDN = (PQCTLV_DEVICE_VOICE_NUMBERS)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x11);
    //USHORT len = le16_to_cpu(pIMEI->TLVLength);

	//snprintf(DeviceIMEI, len, "%s", );
	//memcpy((void *)DeviceIMEI, (const void * )pIMEI->SerialNumberString, len);

    //dbg_time("%s DeviceIMEI -3 : 0x%02x, %d, %s", __func__, pIMEI1->TLVType, pIMEI1->TLVLength, (char * )pIMEI1->SerialNumberString);

    dbg_time("%s msisdn: 0x%02x, %d", __func__, p->TLVType, p->TLVLength);
    dbg_time("%s msisdn: 0x%02x, %d, 0x%02x, %d", __func__, p->Type, p->Length, p->QMUXResult, p->QMUXError);
    dbg_time("%s msisdn -2: 0x%02x, %d", __func__, p1->TLVType, p1->TLVLength);
...
//#ifdef CONFIG_IMEI
#if 1
int requestGetIMEI(void) { //RIL_REQUEST_GET_IMSI
    PQCQMIMSG pRequest;
    PQCQMIMSG pResponse;
    PQMUX_MSG pMUXMsg;
	
	static char DeviceIMEI[16] = {'\0'};
	
    PQCTLV_DEVICE_SERIAL_NUMBER pIMEI;
    ...
    pIMEI = (PQCTLV_DEVICE_SERIAL_NUMBER)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x11);
    USHORT len = le16_to_cpu(pIMEI->TLVLength);
    ...
    dbg_time("%s DeviceIMEI: 0x%02x, %d, %s", __func__, pIMEI->TLVType, pIMEI->TLVLength, DeviceIMEI);

	cm_status.bb_status.write(&(cm_status.bb_status), NULL, DeviceIMEI);
...
int requestGetSIMStatus(SIM_Status *pSIMStatus) { //RIL_REQUEST_GET_SIM_STATUS
    ...
    *pSIMStatus = SIM_ABSENT;
	...
	cm_status.sim_status.write(&(cm_status.sim_status),(char *)SIM_Status_String[*pSIMStatus],NULL,NULL);
...
int requestGetICCID(void) { //RIL_REQUEST_GET_IMSI
    ...
    if (pUimContent != NULL) {
		...
		cm_status.sim_status.write(&(cm_status.sim_status),NULL,DeviceICCID,NULL);
    }
...
int requestGetIMSI(void) { //RIL_REQUEST_GET_IMSI
    ...
    if (pUimContent != NULL) {
		...
		cm_status.sim_status.write(&(cm_status.sim_status),NULL,NULL,DeviceIMSI);
    }
...
int requestRegistrationState2(UCHAR *pPSAttachedState) {
    ...
    *pPSAttachedState = 0;
	...
	cm_status.reg_status.write(&(cm_status.reg_status), 
		&MobileCountryCode, &MobileNetworkCode,
		pPSAttachedState, (char *)pDataCapStr, 0, NULL);
...
int requestQueryDataCall(UCHAR  *pConnectionStatus, int curIpFamily) {
    ...
    *pConnectionStatus = QWDS_PKT_DATA_DISCONNECTED;
	...
	cm_status.write(&cm_status, *pConnectionStatus); 
...
int requestBaseBandVersion(const char **pp_reversion) {
    ...
    if (revId && le16_to_cpu(revId->TLVLength))
    {
		...
		cm_status.bb_status.write(&(cm_status.bb_status), DeviceRevisionID, NULL);
    }
